...
class Constraints_generator():
    ...
    def genConstraints_of_forwardRound(self, r):
        ...
        constr = constr + MITMPreConstraints.equalConstraints(SC_Whirlpool(input1_round), input1_mix)
        constr = constr + MITMPreConstraints.equalConstraints(SC_Whirlpool(input2_round), input2_mix)
        ...
        for j in range(8):
            input1_col = row(input1_mix, j)
            input2_col = row(input2_mix, j)
            out1_col = row(input1_AK, j)
            out2_col = row(input2_AK, j)          
    ...
    def genConstraints_of_backwardRound(self, r):
        ...
        if r < self.TR-1:
            ...
            for j in range(64):
                constr = constr + MITMPreConstraints.XOR_forward([out1_round[j],T1[j]],[out2_round[j],T2[j]],input1_AK[j],input2_AK[j],CD_T[j])            

        # - Constraints for MixColumns          
        for j in range(8):
            input1_col = row(input1_AK, j)
            input2_col = row(input2_AK, j)
            out1_col = row(input1_mix, j)
            out2_col = row(input2_mix, j)
    ...
    def genConstraints_additional(self):
        ...
        for j in range(8):
            constr = constr + [Gsum[j] + ' - ' + BasicTools.MinusTerm(row(Input1_mat, j) + row(out2_mat, j)) + ' = 0']
...
#       
    ...
    def genModel(self, filename):
        ...
        constr = constr + ['2 GDeg1 - Gt >= 2']
        constr = constr + ['2 GDeg2 - Gt >= 2']
...
def cmd():
    rd = open('./Model/Result_6_notatlast.txt', 'w')
