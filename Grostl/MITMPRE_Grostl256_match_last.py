...
class Constraints_generator():
    ...
    def genConstraints_additional(self):
        ...
        Deg2 = 'GDeg2'
        if len(CD2_b) > 0:
            constr = constr + ['GDeg1' + ' - ' + BasicTools.MinusTerm(d1) + ' + ' + BasicTools.plusTerm(CD2_b) + ' = 0']
        else:
            constr = constr + ['GDeg1' + ' - ' + BasicTools.MinusTerm(d1) + ' = 0']
            
        if len(CD1_f) > 0:
            constr = constr + ['GDeg2' + ' - ' + BasicTools.MinusTerm(d2) + ' + ' + BasicTools.plusTerm(CD1_f) + ' = 0']
        else:
...
def cmd():
    rd = open('./Model256/Result_6.txt', 'w')
    rd.write('TR, ini_r, mat_r: d1, d2, m' + '\n' )
    for TR in range(6, 7):
        for ini_r in range(0, TR):
            for mat_r in range(TR-1, TR):
                if mat_r != ini_r:                        
