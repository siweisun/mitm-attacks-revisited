...
class Vars_generator:
    ...
    def genVars_T1():
        return  ['T1_' + str(j) for j in range(64)]
    
    def genVars_T2():
        return  ['T2_' + str(j) for j in range(64)]
        
    def genVars_ConsumedDeg_of_AKT():
        return ['CDeg_T' + str(j) for j in range(64)]        
...
class Constraints_generator():
    ...
    def genConstraints_of_forwardRound(self, r):
        ...
        Allone1 = Vars_generator.genVars_AllOne1_of_MixColumn(r) #Allone1 = 1 iff all input are 1
        ...
        constr = constr + MITMPreConstraints.equalConstraints(SR_Groestl(input1_round), input1_mix)
        constr = constr + MITMPreConstraints.equalConstraints(SR_Groestl(input2_round), input2_mix)
    ...
    def genConstraints_of_backwardRound(self, r):
        ...
        for j in range(8):
            input1_col = column(input1_AK, j)
            input2_col = column(input2_AK, j)
    ...
    def genConstraints_matching_round(self): 
        ...
        if self.mat_r < self.TR - 1:
            ...
            for i in range(0,8):
                for j in range(4,8):
                    constr = constr + [input1_AK[8*i+j] + ' - ' + out1_round[8*i+j] + ' = 0']
                    constr = constr + [input2_AK[8*i+j] + ' - ' + out2_round[8*i+j] + ' = 0']  
    ...
    def genConstraints_additional(self):
        ...
        for i in range(0,8):
            for j in range(0,8):
                if j < 4:
                    constr = constr + [T1[8*i+j] + ' = 0']
                    constr = constr + [T2[8*i+j] + ' = 0']  
                else:
                    constr = constr + [T1[8*i+j] + ' - ' + T2[8*i+j] + ' = 0']
        ...
        constr = constr + ['2 GDeg1' +  ' < 32']
        constr = constr + ['2 GDeg2' +  ' < 32']        
    ...
    def genConstraints_total(self):
        ...
        # if self.mat_r > self.ini_r:
            # for i in range(0,8):
                # for j in range(0,4):
                    # constr = constr + [input1_AK[8*i+j] + ' = 0']
                    # constr = constr + [input2_AK[8*i+j] + ' = 0']
        # else:
            # for i in range(0,8):
                # for j in range(0,4):
                    # constr = constr + [input1_round[8*i+j] + ' = 0']
                    # constr = constr + [input1_round[8*i+j] + ' = 0']        
    ...
    def genModel(self, filename):
        ...
        fid = open('./Model256/TR' + str(self.TR) + '_ini' + str(self.ini_r) + '_matr' + str(self.mat_r) + '.lp', 'w')
...
def cmd():
    rd = open('./Model256/Result_6_notatlast.txt', 'w')
    rd.write('TR, ini_r, mat_r: d1, d2, m, t' + '\n' )
    for TR in range(6, 7):
        for ini_r in range(0, TR):
            for mat_r in range(0, TR-1):
                if mat_r != ini_r:                        
                    name = './Model256/TR' + str(TR) + '_ini' + str(ini_r) + '_matr' + str(mat_r)
